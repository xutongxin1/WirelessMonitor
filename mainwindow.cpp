...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
  ...
  for (int i = 1; i <= DeviceCount; i++) {
    ...
    connect(DeviceSelect[i]->Button, &QPushButton::pressed, this, [=] { MainWindow::DeviceExchange(i); });
  }
...
void MainWindow::DeviceWindowsInit() {
  ...
  for (int DeviceNum = 1; DeviceNum <= DeviceCount; DeviceNum++)  // 设备遍历初始化
  {
    ...
    struct DevicesInfo tmp
        {
            .windowsNum = Cfg->GetMainCfg("/Device " + QString::number(DeviceNum) + "/win").toInt(),
            .TabIndex = ui->TabStackedWidget->addWidget(NewTab), .TabWidget = NewTab
        };
    ...
    for (int WinNum = 1; WinNum <= DevicesInfo[DeviceNum].windowsNum; WinNum++)  // 窗口遍历初始化
    {
      ...
      switch (WinType) {
        case 1:DevicesWindowsInfo[DeviceNum][WinNum].type = Channel_Configuration;  // 结构体初始化
          DevicesWindowsInfo[DeviceNum][WinNum].widget = new ChannelConfiguration(
              DeviceNum, Cfg->configDeviceIni[0], Cfg->configDeviceIni[DeviceNum], &parentInfo);
          DevicesWindowsInfo[DeviceNum][WinNum].index =
              ui->FunctionWindow->addWidget(DevicesWindowsInfo[DeviceNum][WinNum].widget);
          DevicesInfo[DeviceNum].TabWidget->addTab("通道配置");  // 添加tab栏
          break;
        case 50:DevicesWindowsInfo[DeviceNum][WinNum].type = XCOM;  // 结构体初始化
          DevicesWindowsInfo[DeviceNum][WinNum].widget =
              new ComTool(DeviceNum, WinNum, Cfg->configDeviceIni[DeviceNum], &parentInfo);
          DevicesWindowsInfo[DeviceNum][WinNum].index =
              ui->FunctionWindow->addWidget(DevicesWindowsInfo[DeviceNum][WinNum].widget);
        ...
        case 51: {  // 记录下相应的变量，方便提取类的成员变量charts
          ...
          Charts *test1 = new Charts(DeviceNum, WinNum, Cfg->configDeviceIni[DeviceNum], &parentInfo);
        ...
        case 52:DevicesWindowsInfo[DeviceNum][WinNum].type = Data_Circulation;  // 结构体初始化
          DevicesWindowsInfo[DeviceNum][WinNum].widget =
              new DataCirculation(DeviceNum, WinNum, Cfg->configDeviceIni[DeviceNum], &parentInfo);
          DevicesWindowsInfo[DeviceNum][WinNum].index =
              ui->FunctionWindow->addWidget(DevicesWindowsInfo[DeviceNum][WinNum].widget);
          DevicesInfo[DeviceNum].TabWidget->addTab("数据流过滤器配置");  // 添加tab栏
          break;
        case 201:DevicesWindowsInfo[DeviceNum][WinNum].type = TCP_Bridge_Configuration;  // 结构体初始化
          DevicesWindowsInfo[DeviceNum][WinNum].widget =
              new TCPBridgeConfiguration(DeviceNum, WinNum, Cfg->configDeviceIni[DeviceNum], &parentInfo);
          DevicesWindowsInfo[DeviceNum][WinNum].index =
              ui->FunctionWindow->addWidget(DevicesWindowsInfo[DeviceNum][WinNum].widget);
          DevicesInfo[DeviceNum].TabWidget->addTab("串口桥配置");  // 添加tab栏
          break;
        case 202:DevicesWindowsInfo[DeviceNum][WinNum].type = TCP_Com;  // 结构体初始化
          DevicesWindowsInfo[DeviceNum][WinNum].widget =
              new TCPCom(DeviceNum, WinNum, Cfg->configDeviceIni[DeviceNum], &parentInfo);
          DevicesWindowsInfo[DeviceNum][WinNum].index =
              ui->FunctionWindow->addWidget(DevicesWindowsInfo[DeviceNum][WinNum].widget);
...
void MainWindow::DeviceWindowsExchange(int DeviceNum, int WinNum) {
  ...
  if (DevicesInfo[DeviceNum].configStep < WinNum) {
    //TODO:添加提醒用户完成上一步骤
    qDebug("Switch Failed");
    DevicesInfo[DeviceNum].TabWidget->setCurrentTab(DevicesInfo[DeviceNum].current_window - 1,
                                                    false);//把高亮回到该窗口（但受限于库底层，我无法解决它）
    return;
  }
  ui->FunctionWindow->setCurrentIndex(DevicesWindowsInfo[DeviceNum][WinNum].index);
  DevicesInfo[DeviceNum].current_window = WinNum;
}
