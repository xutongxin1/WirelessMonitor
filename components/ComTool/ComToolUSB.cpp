...
void ComTool::ComboChange(int num) {
	//防止没COM了
	if (ui_->COMCombo->currentText().isEmpty())
		return;
	...
	//更新配置文件
	cfg_->beginGroup("main");
	cfg_->setValue("H_UUID", ComUUID);
	cfg_->endGroup();
...
void ComTool::ComHistoryGet() {
	cfg_->beginGroup("main");
	QString H_UUID = cfg_->value("H_UUID").toString();
	foreach(QString tmp, my_serialportinfo_)
	{
		if (QCryptographicHash::hash(ComNameAndSymbolic.value(GetTrueComName(tmp), "error").toUtf8(),
		                             QCryptographicHash::Md5).toHex() == H_UUID)
		{
			ui_->COMCombo->setCurrentText(tmp);
			break;
		}
	}
	cfg_->endGroup();
