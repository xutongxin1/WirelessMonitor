 ...
 * TODO:ui修改
 */
ComTool::ComTool(int device_num,
                 int win_num,
                 QSettings *cfg,
    ...
    connect(timer_for_port_,
            &QTimer::timeout,
            this,
            &ComTool::ReflashComCombo);
...
/// 往日志区添加数据
/// \param ComID 串口ID
/// \param type 类型，1是str接收，2是hex接收,3是str发送，4是hex发送
/// \param data 数据
/// \param data 是否转码得到
/// \param clear 是否清空
void ComTool::AppendToTxtMain(char ComID,
                              char type,
                              const QString &data,
                              bool isTransfer) {
    ...
    if (isTransfer)
        str_type = "【解析后】" + str_type;
    // ui_->txtMain->
    //     setTextColor(colorArray[ComID * 4 + type - 1]);
...
void ComTool::ProcessData(QByteArray main_serial_recv_data) {
    receive_count_ = receive_count_ + static_cast<int>(main_serial_recv_data.size());
    QString buffer;
...
void ComTool::SendData() {
    ...
    if (ui_->ckHexSend->isChecked()) {
        ...
        this->AppendToTxtMain(0, 3, data);
    }

    my_serialport_->write(buffer);

    send_count_ = send_count_ + static_cast<int>(buffer.size());
...
void ComTool::showEvent(QShowEvent *event) {
    if (!hasInit) {
        SizeSplitterWithFactor(ui_->SendSplitter,
                               true,
                               0.6,
                               0.4);
        SizeSplitterWithFactor(ui_->LRSplitter,
                               false,
                               0.7,
                               0.3);
        hasInit = true;
