...
void ChartsNext::LoadInfo() {
    ...
    for (QList<DataNode>::iterator i = data_pool_.begin(); i != data_pool_.end(); ++i, count++) {
        // QTable添加文字，控件
        ui_chart_->line_table->setItem(count, 0, new QTableWidgetItem(data_pool_.at(count).data_name));     // 添加变量名
        ui_chart_->line_table->setCellWidget(count, 1, line_info_[count].choose_color);                         // 颜色选择控件
        ui_chart_->line_table->setCellWidget(count, 2, line_info_[count].check_visible);                        // 显示选择框
...
//        cfg_->beginGroup("value" + QString::number(i + 1));
        ...
        cfg_->setValue("Name", data_pool_.at(i).data_name);     /// 变量名
        cfg_->setValue("Color", data_pool_.at(i).line_color);      /// 颜色
        cfg_->setValue("Visible", data_pool_.at(i).is_visible);   /// 是否可见
        cfg_->setValue("Width", data_pool_.at(i).line_width);   /// 是否可见
        cfg_->endGroup();
    }
...
void ChartsNext::GetConstructConfig() {
    ...
    for (int i = 0; i < data_pool_.size(); i++) {
        cfg_->beginGroup(group_name_);
//        cfg_->beginGroup("value" + QString::number(i + 1));
        data_pool_[i].data_name = cfg_->value("Name").toString();
        QVariant value = cfg_->value("Color");        // 读出来的是color的hex值
        QColor color = value.value<QColor>();           // 这是一个QColor(ARGB 1, 1, 0, 0)
        data_pool_[i].line_color = color;
        data_pool_[i].is_visible = cfg_->value("Visible").toBool();
        data_pool_[i].line_width = cfg_->value("Width").toInt();
        cfg_->endGroup();
    }
