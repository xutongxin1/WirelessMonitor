...
ChartsNext::ChartsNext(int DeviceNum, int winNum, QSettings *cfg, ToNewWidget *parentInfo, QWidget *parent) :
    RepeaterWidget(parent),
    uiChart(new Ui::charts_next) {
...
//    timerChart->start();
    ...
    connect(DataReceiverNext::getInstance(), &DataReceiverNext::oneDataReady, this, [&] { ShowLine(uiChart->widget); });
...
void ChartsNext::ShowLine(QCustomPlot *customPlot) {
...
void ChartsNext::on_pushButton_clicked() {
    if (checked == 0) {
        //开启，不可以放缩和移动
        //qDebug() << "开始画图" << endl;
        uiChart->start_stop->setText("暂停显示");
        ...
        //关闭，可以放缩和移动
        uiChart->start_stop->setText("开始显示");
        //timerChart->stop();
        uiChart->widget->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom | QCP::iSelectPlottables);
        hide_flag = 0;
        checked = 0;
 ...
 * 成功返回1，失败0
*****/
bool ChartsNext::registerData(const QString &addName, DataType datatype) {
 ...
 * 成功返回1，失败0
*****/
bool ChartsNext::antiRegisterData(QString addName) {
