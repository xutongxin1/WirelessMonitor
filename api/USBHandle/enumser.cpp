...
_Return_type_success_(return != false) bool CEnumerateSerial::RegQueryValueString(
    ...
    // // 查询键信息
    // status = RegQueryInfoKeyA(key, NULL, NULL, NULL, NULL, NULL, NULL, &valueCount, &maxValueNameLen, &maxValueDataLen,
    //                           NULL, NULL);
    // if (status != ERROR_SUCCESS) {
    //     printf("Unable to query key info\n");
    //     RegCloseKey(key);
    //     return 1;
    // }
    //
    ...
    // BYTE *valueData = (BYTE *) malloc(maxValueDataLen + 1); // 确保有足够的空间存储数据
    //
    // for (DWORD i = 0; i < valueCount; i++) {
    //     DWORD nameLen = maxValueNameLen + 1;
    //     DWORD dataLen = maxValueDataLen + 1;
    //     DWORD type;
    //     status = RegEnumValueA(key, i, valueName, &nameLen, NULL, &type, valueData, &dataLen);
    //     if (status != ERROR_SUCCESS) {
    //         printf("Unable to enum value\n");
    //         break;
    //     }
    //
    //     // 假设值数据是字符串类型
    //     if (type == REG_SZ) {
    //         valueData[dataLen] = '\0'; // 确保字符串以null终止
    //         printf("Value #%d: %s = %s\n", i + 1, valueName, (char *) valueData);
    //     } else {
    //         printf("Value #%d: %s = [Non-string data]\n", i + 1, valueName);
    //     }
    // }
    //
    // free(valueName);
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryRegistryPortName(
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryUsingSetupAPI(
    ...
    while (bMoreItems) {
        ...
        if (bMoreItems) {
            ...
            auto deviceKeyHandle = SetupDiOpenDevRegKey(hDevInfoSet, &devInfo, DICS_FLAG_GLOBAL, 0, DIREG_DEV,
                                                        KEY_QUERY_VALUE);
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryDeviceDescription(
...
#pragma warning(suppress: 26446 26490)
    if (!SetupDiGetDeviceRegistryProperty(hDevInfoSet, &devInfo, SPDRP_DEVICEDESC, &dwType, friendlyName.data(), dwSize,
                                          &dwSize))
        return false;
    if (dwType != REG_SZ) {
        SetLastError(ERROR_INVALID_DATA);
...
#pragma warning(suppress: 26429)
_Return_type_success_(return != false) bool CEnumerateSerial::IsNumeric(
    _In_z_ LPCWSTR pszString, _In_ bool bIgnoreColon) noexcept {
...
#pragma warning(suppress: 26481 26486 26489)
                if ((_tcsnicmp(pPortInfo->pPortName, _T("COM"), 3) == 0) &&
                    IsNumeric(&(pPortInfo->pPortName[3]), true)) {
                    //Work out the port number
...
#pragma warning(suppress: 26490)
    ...
    hr = pLocator->ConnectServer(_bstr_t(R"(\\.\root\cimv2)"), nullptr, nullptr, nullptr, 0, nullptr, nullptr,
                                 &pServices);
    if (FAILED(hr))
        return hr;
    ...
    hr = pServices->CreateInstanceEnum(_bstr_t("Win32_SerialPort"), WBEM_FLAG_RETURN_WBEM_COMPLETE, nullptr,
                                       &pClassObject);
    if (FAILED(hr))
        return hr;
...
#pragma warning(suppress: 26446)
    const LONG nStatus{
        ComDBGetCurrentPortUsage(hComDB, portBytes.data(), dwMaxPortsReported, CDB_REPORT_BYTES, &dwMaxPortsReported)
    };
    if (nStatus != ERROR_SUCCESS) {
        ComDBClose(hComDB);
        SetLastError(nStatus);
...
_Return_type_success_(return != false) bool CEnumerateSerial::UsingRegistry(_Inout_ CNamesArray &ports) {
    ...
    nStatus = RegQueryInfoKey(serialCommKey, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                              &dwMaxValueNameLen, nullptr, nullptr, nullptr);
    if (nStatus != ERROR_SUCCESS) {
        SetLastError(nStatus);
        return false;
    }
...
#pragma warning(suppress: 26446)
        valueName[0] = _T('\0');
#pragma warning(suppress: 26446)
        bContinueEnumeration = (RegEnumValue(serialCommKey, dwIndex, valueName.data(), &dwValueNameSize, nullptr,
